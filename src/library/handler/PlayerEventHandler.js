...
class PlayerEventHandler extends Handler {
  ...
  execute(bot, session, type, payload, logFunc) {
    ...
    if (type == 'player.join') bot.sendMessage(`:inbox_tray: ${player} has joined`).catch(() => console.error('bot.sendMessage error @ PlayerEventHandler.js#33'));
      ...
      else bot.sendMessage(`:boot: ${player} has been kicked by ${responsible}`).catch(() => console.error('bot.sendMessage error @ PlayerEventHandler.js#40'));
    } else if (type == 'player.ban') {
      let responsible = this.escape(payload.responsible || 'Console');

      if (reason)
        bot
          .sendMessage(`:hammer_pick: ${player} has been banned by ${responsible} (reason: ${reason})`)
          .catch(() => console.error('bot.sendMessage error @ PlayerEventHandler.js#46'));
      else bot.sendMessage(`:hammer_pick: ${player} has been banned by ${responsible}`).catch(() => console.error('bot.sendMessage error @ PlayerEventHandler.js#48'));
    } else if (type == 'player.quit') {
      if (!payload.type) return;

      let quitType = this.escape(payload.type);

      if (reason) bot.sendMessage(`:door: ${player} has left [${quitType}] (reason: ${reason})`).catch(() => console.error('bot.sendMessage error @ PlayerEventHandler.js#57'));
      else bot.sendMessage(`:door: ${player} has left [${quitType}]`).catch(() => console.error('bot.sendMessage error @ PlayerEventHandler.js#59'));
    ...
    } else if (type == 'player.mute') bot.sendMessage(`:mute: ${player} has been muted`).catch(() => console.error('bot.sendMessage error @ PlayerEventHandler.js#70'));
    else if (type == 'player.unmute') bot.sendMessage(`:speaker: ${player} has been unmuted`).catch(() => console.error('bot.sendMessage error @ PlayerEventHandler.js#72'));
    else if (type == 'player.finish') {
      if (!payload.rank || !this.ranks[payload.rank]) return;
      ...
      if (typeof payload.position !== 'number' || !payload.time) return;

      let time = this.escape(payload.time);
      bot
        .sendMessage(`:trophy: ${player} has made a new #${payload.position} toptime **[${time}]**`)
        .catch(() => console.error('bot.sendMessage error @ PlayerEventHandler.js#85'));
    }
  }
}
